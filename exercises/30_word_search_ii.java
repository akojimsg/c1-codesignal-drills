...
class Solution {
    ...
    public List<String> findWords(char[][] board, String[] words) {
        Node root = new Node(); for (String w : words) insert(root, w);
    ...
    private void insert(Node root, String w) {
        for (char c : w.toCharArray()) {
            int k=c-'a'; if(root.ch[k]==null) root.ch[k]=new Node(); root=root.ch[k];
        } root.word=w;
    }
    private void dfs(char[][] b, int r, int c, Node node, List<String> res) {
        if (r<0||c<0||r==b.length||c==b[0].length||b[r][c]=='#') return;
        char ch=b[r][c]; Node next=node.ch[ch-'a']; if (next==null) return;
        if (next.word!=null) { res.add(next.word); next.word=null; }
        b[r][c]='#'; dfs(b,r+1,c,next,res); dfs(b,r-1,c,next,res);
        dfs(b,r,c+1,next,res); dfs(b,r,c-1,next,res); b[r][c]=ch;
    }
}
